...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var apiKey = 'AIzaSyDzoGeidYEMlgYGRvuVUvjvOebicfGPoQw'; // 구글 번역 API 키를 여기에 입력
    ...
    function translatePage(targetLang) {
      var elementsToTranslate = document.body.innerText; // 번역할 텍스트
      fetch(`https://translation.googleapis.com/language/translate/v2?key=${apiKey}`, {
        method: 'POST',
        body: JSON.stringify({
          q: elementsToTranslate,
          target: targetLang,
        }),
        headers: {
      ...
      .then(data => {
        // 번역된 결과를 페이지에 반영
        document.body.innerHTML = data.data.translations[0].translatedText;
      })
      .catch(error => {
