...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var apiKey = 'AIzaSyCFC7Gk_htnGl6JZqXsDw9yjT7JJiwqgQI'; // 구글 번역 API 키를 여기에 입력
    ...
    function fetchCountryCode() {
      return fetch('https://ipapi.co/json/')
       // .then(response => response.json())  // ★★★ 임시 주석처리함 (초기 로드시 국가 자동인식 방지)
    ...
    // 구글 번역 API 호출하여 페이지 번역하는 함수
function translatePage(targetLang) {
  // 번역할 요소들을 선택합니다
  var elementsToTranslate = document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, span, strong, a, b, button, label, li, div, dd, dt, td');
  ...
  elementsToTranslate.forEach(function(element) {
    if (element.childNodes.length === 1 && element.childNodes[0].nodeType === Node.TEXT_NODE) {
      ...
      // 요소가 다른 요소를 포함하는 경우 (예: a 태그 내의 span)
            ...
            if ($k == 0) { ?>
              <div @click.away="open = false" class="relative" x-data="{ open: false }">
                <button @click="open = !open" class="flex flex-row items-center w-full px-3 py-1 mt-2 text-left rounded-lg lg:w-auto lg:mt-0 focus:text-indigo-500 hover:bg-indigo-200 focus:bg-indigo-200 focus:outline-none focus:shadow-outline">
                  <span class="notranslate"><?php echo $row['me_name'] ?></span>
              ...
              <?php } else { ?>
                <div @click.away="open = false" class="relative" x-data="{ open: false }">
                  <button @click="open = !open" class="flex flex-row items-center w-full px-3 py-1 mt-2 text-left rounded-lg lg:w-auto lg:inline lg:mt-0 hover:text-indigo-900 focus:text-indigo-900 hover:bg-indigo-200 focus:bg-indigo-200 focus:outline-none focus:shadow-outline text-base">
                    <span class="notranslate">Information</span>
                  ...
                  <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg lg:w-48">
                    <div class="dropdownItem px-2 py-2 rounded-md shadow relative z-50 notranslate">
                      <a class="block px-4 py-1 mb-0.5 rounded-lg hover:text-black focus:text-blxk hover:bg-indigo-200 focus:bg-indigo-200 focus:outline-none focus:shadow-outline" href="<?php echo G5_URL ?>/notice" target="_top">Notice board</a>
                      ...
                      <a class="block px-4 py-1 mb-0.5 rounded-lg hover:text-black focus:text-blxk hover:bg-indigo-200 focus:bg-indigo-200 focus:outline-none focus:shadow-outline" href="<?php echo G5_BBS_URL ?>/faq.php" target="_top">FAQ</a>
